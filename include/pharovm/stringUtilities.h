 ...
 */
VM_EXPORT void vm_string_append_into(char *dest, const char *source, size_t destBufferSize);

/**
 * This concatenates two different strings into a newly allocated string.
 * The result must be freed with free
 */
VM_EXPORT char* vm_string_concat(const char *first, const char *second);
 ...
 * The result must be freed with free.
 */
VM_EXPORT uint16_t* vm_string_convert_utf8_to_utf16(const char *utf8String);

/**
 * Converts an utf-8 string into an utf-16 string. This is typically used for calling win32 APIs
 * The result must be freed with free.
 */
VM_EXPORT char* vm_string_convert_utf16_to_utf8(const uint16_t *utf16String);
